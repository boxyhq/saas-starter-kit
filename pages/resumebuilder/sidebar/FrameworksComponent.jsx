...
const FrameworksComponent = () => {
  ...
  const handleAddFramework = () => {
    if (newFramework && newLevel > 0) {
      setFrameworks([...frameworks, { id: `${frameworks.length + 1}`, name: newFramework, level: newLevel }]);
      setNewFramework('');
      setNewLevel(0);
    }
  };
  ...
  return (
    <div>
              ...
              {frameworks.map(({ id, name, level }, index) => (
                <Draggable key={id} draggableId={id} index={index}>
                  ...
                  >
                      ...
                      <Button variant="danger" size="sm" onClick={() => handleDeleteFramework(id)}>X</Button>
                    </ListGroup.Item>
      ...
      <Button onClick={handleAddFramework} className="mt-3">Add</Button>
    </div>
  );
};
