...
export default async function handler(
  ...
  try {
    ...
    const { mdrId, compilationId } = req.query as {
      mdrId: string;
    ...
    const compilation = await prisma.mdrCompilation.findUniqueOrThrow({
      where: { id: compilationId, mdrProjectId: mdrId },
    });

    let downloadUrl: string | null = null;

    if (compilation.status === 'COMPLETE' && compilation.s3Key) {
    ...
    res.status(200).json({ data: { ...compilation, downloadUrl } });
  } catch (error: any) {
