...
Duplicate `.env.example` to `.env`. And fill three placholders of the `DATABASE_URL=...` with the `environment` settings from the `docker-compose.db.yml`
...
##### B. Start as Docker container

Start as a dev container
